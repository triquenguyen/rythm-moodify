...
async function processRawText(
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    if (status === 'COMPLETED') {
      ...
      const predictions = await getEmotions(jobId);
...
async function getEmotions(jobId) {
  const options = buildHumeRequestOptions('GET');
  ...
  return json[0].results.predictions[0].models.language.grouped_predictions[0].predictions[0].emotions;
}
...
export const fetchEmotions = action({
