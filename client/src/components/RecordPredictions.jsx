...
const RecordPredictions = () => {
  ...
  async function processRawText(fileName, language, languageModelConfig) {
    const MAX_RETRIES = 5; // adjust the number of retries here
    const INITIAL_DELAY_MS = 1000; // starting with 1 second delay

    let delay = INITIAL_DELAY_MS;
    const jobId = await startJob(fileName, language, languageModelConfig);
  ...
  async function startJob(
    ...
    const body = JSON.stringify({
      file: `data:audio/x-m4a;base64,${fileName}`,
  ...
  const handleSubmit = async (e) => {
    const data = await processRawText(record, language, languageModelConfig)
    setPredictions(data)
  }
  ...
  return (
    <div>
      ...
      <button onClick={handleSubmit} className='px-3 py-2 border border-[#3cb2f0] rounded-md hover:bg-[#3cb2f0]'>Submit</button>
    </div>
  )
}
